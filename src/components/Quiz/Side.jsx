...
const Side = ({
  ...
  const goToQues = (quesNo) => {
    let nextQues = { ...allQuestions[quesNo] };
    if (nextQues.status === "not-visited") {
      nextQues = {
        ...nextQues,
        status: "not-answered",
      };
  ...
  return (
    <>
      <Wrapper isSideBarOpen={isSideBarOpen} translate={translate}>
        <Name>
          <img src={info ? info.image?info.image : "/images/user.png":"/images/user.png"} alt="avatar" />
          <p>{user.name[0]}</p>
        ...
        <Stats>
          ...
          <StatCount count={notVisitedCount} text="Not Visited" type="not-visited" />
          <StatCount count={notAnsweredCount} text="Not Answered" type="not-answered" />
          <StatCount
