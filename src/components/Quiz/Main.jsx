import { useState, useEffect } from "react";
import styled from "styled-components";
import { OutlinedButton } from "./Styled";
import ArrowBackIosIcon from "@material-ui/icons/ArrowBackIos";
import Loader from "../../pages/Loader";
import { baseURL } from "../../Apis";
import Markdown from "../Util/Markdown"


let tempCurrentQues = null;

const Main = ({
  isSideBarOpen,
  handleToggleClick,
  currentQuestion,
  setCurrentQuestion,
  allQuestions,
  setAllQuestions,
  answer,
  setAnswer,
  sectionArray,
  quizId,
}) => {
  const [activeSection, setActiveSection] = useState(sectionArray[0].name);
  const [error, setError] = useState(null);
  const [isLoaded, setIsLoaded] = useState(true);
  const [natAnswer, setnatAnswer] = useState("");
  useEffect(() => {
    if (currentQuestion) {
      const section = sectionArray.filter((section) => {
        return (
          section.startIndex <= currentQuestion.index &&
          section.startIndex + section.quesCount > currentQuestion.index
        );
      });
      setActiveSection(section[0].name);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [currentQuestion]);

  const sendAnswer = (question) => {
    setIsLoaded(false);
    setError(null);

    const answerObj = {
      quesId: `${question._id}`,
      markedAns: question.answered,
    };
    fetch(`${baseURL}/quiz/${quizId}`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${localStorage.getItem("token")}`,
      },
      body: JSON.stringify(answerObj),
    })
      .then((res) => res.json())
      .then(
        (result) => {
          setIsLoaded(true);
        },
        (error) => {
          setError(error);
        }
      );
  };

  const clearAnswer = (question) => {
    setIsLoaded(false);
    setError(null);

    fetch(`${baseURL}/quiz/${quizId}?quesId=${question._id}`, {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${localStorage.getItem("token")}`,
      },
    })
      .then((res) => res.json())
      .then(
        (result) => {
          setIsLoaded(true);
        },
        (error) => {
          setError(error);
        }
      );
  };

  const saveHandler = () => {
    if (!currentQuestion) {
      return;
    }

    if (answer.length !== 0) {
      tempCurrentQues = {
        ...currentQuestion,
        answered: answer,
        status: "answered",
      };
      sendAnswer(tempCurrentQues);
    } else {
      tempCurrentQues = {
        ...currentQuestion,
        status: "not-answered",
      };
    }
    setCurrentQuestion(tempCurrentQues);
    updateAllQuesArray();
    nextQuestion();
  };

  const saveMarkHandler = () => {
    if (!currentQuestion) {
      return;
    }

    if (answer.length !== 0) {
      tempCurrentQues = {
        ...currentQuestion,
        answered: answer,
        status: "marked-answered",
      };

      sendAnswer(tempCurrentQues);
      setCurrentQuestion(tempCurrentQues);
      updateAllQuesArray();
      nextQuestion();
    }
  };

  const markHandler = () => {
    if (!currentQuestion) {
      return;
    }

    tempCurrentQues = {
      ...currentQuestion,
      answered: [],
      status: "marked",
    };

    setCurrentQuestion(tempCurrentQues);
    updateAllQuesArray();
    nextQuestion();
  };

  const clearHandler = () => {
    if (!currentQuestion) {
      return;
    }

    if (answer.length !== 0) {
      tempCurrentQues = {
        ...currentQuestion,
        answered: [],
        status: "not-answered",
      };
      clearAnswer(tempCurrentQues);
    } else {
      tempCurrentQues = {
        ...currentQuestion,
        status: "not-answered",
      };
    }

    setCurrentQuestion(tempCurrentQues);
    updateAllQuesArray();
    nextQuestion();
  };

  const updateAllQuesArray = () => {
    let quesArray = allQuestions.map((question) => {
      if (question.index === tempCurrentQues.index) {
        return tempCurrentQues;
      } else {
        return question;
      }
    });
    setAllQuestions(quesArray);
  };

  const nextQuestion = () => {
    if (currentQuestion.index + 1 < allQuestions.length) {
      setCurrentQuestion((prevQuestion) => {
        let nextQuestion = allQuestions[prevQuestion.index + 1];
        if (nextQuestion.status === "not-visited") {
          nextQuestion = {
            ...nextQuestion,
            status: "not-answered",
          };
        }
        return nextQuestion;
      });
    } else {
      //submit test
    }
  };

  if (error) {
    return <div>Error: {error.message}</div>;
  } else {
    return (
      <MainCont isSideBarOpen={isSideBarOpen}>
        <Toggle onClick={handleToggleClick}>
          <ArrowBackIosIcon />
        </Toggle>
        <SectionHeader>
          <div className="sections">
            <StyledLabel>Sections :</StyledLabel>
            {sectionArray.map((section, index) => (
              <SectionButton
                active={activeSection === section.name} //TODO
                onClick={() => {
                  setActiveSection(section.name);
                  if (section && section.startIndex !== null) {
                    tempCurrentQues =
                      allQuestions.length === 0
                        ? null
                        : allQuestions[section.startIndex];
                  }
                  if (
                    tempCurrentQues &&
                    tempCurrentQues.status === "not-visited"
                  ) {
                    tempCurrentQues = {
                      ...tempCurrentQues,
                      status: "not-answered",
                    };
                  }
                  setCurrentQuestion(tempCurrentQues);
                  updateAllQuesArray();
                }}
                key={index}
              >
                <span className="laptop">{section.name}</span>
                <span className="phone">{section.name[0]}</span>
              </SectionButton>
            ))}
          </div>
          <div className="language">
            <StyledLabel htmlFor="language">View in :</StyledLabel>
            <StyledSelect id="language" name="language">
              <option value="english">English</option>
              <option value="hindi">Hindi</option>
            </StyledSelect>
          </div>
        </SectionHeader>
        {currentQuestion && (
          <MarkingScheme>
            <StyledLabel>Marking Scheme :</StyledLabel>
            <div>
              Correct
              <span className="hide-for-small-screen"> Response</span> :
              <span>+{currentQuestion.marking}</span>
            </div>
            <div>
              Incorrect <span className="hide-for-small-screen"> Response</span>
              : <span>-{currentQuestion.negMarking}</span>
            </div>
          </MarkingScheme>
        )}
        <>
          <QuestionElement
            answer={answer}
            currentQuestion={currentQuestion}
            setAnswer={setAnswer}
            natAnswer={natAnswer}
            setnatAnswer={setnatAnswer}
          />
          {!isLoaded && <Loader />}
        </>
        <ButtonContainer>
          <Button onClick={saveHandler}>Save & Next</Button>
          <Button onClick={saveMarkHandler}>Save & Mark for Review</Button>
          <Button onClick={markHandler}>Mark for Review & Next</Button>
          <Button onClick={clearHandler}>Clear & Next</Button>
        </ButtonContainer>
      </MainCont>
    );
  }
};

const QuestionElement = ({ answer, currentQuestion, setAnswer, natAnswer, setnatAnswer }) => {
  

  const handelAnswer = (index) => {
    if (currentQuestion.questionType === 0) {
      setAnswer([index]);
    } else if (currentQuestion.questionType === 1) {
      setAnswer([...answer, index]);
    } else {
      setnatAnswer(index.target.value);
      setAnswer([natAnswer]);
    }
  };
  const addCode = (value) => {
    if (value === "del") {
      let final = natAnswer.slice(0, -1);
      setnatAnswer(final);
      setAnswer([final]);
    } else if (value === "clear") {
      setnatAnswer("");
      setAnswer([]);
    } else {
      let final = natAnswer;
      final = final + value;
      setnatAnswer(final);
      setAnswer([final]);
    }
  };

  const [ques, setques] = useState(null)

  useEffect(() => {
    const fun = (val)=>{
      return <Markdown source={val} />
    }
    // if(currentQuestion.questionType!==2){
    //   currentQuestion.options.forEach((ele)=>{
    //     setans([...ans, fun(ele)])
    //   })
    // }
    setques(fun(currentQuestion.question))
  }, [currentQuestion.question])

  return (
    currentQuestion && (
      <QuestionContainer>
        <QuestionHeader>
          <div className="number">
            Question <span>{currentQuestion.index + 1}</span>
          </div>
          <div className="type">
            {currentQuestion.questionType === 0
              ? "Single Choice Type Question"
              : currentQuestion.questionType === 1
              ? "Multiple Correct Type Question"
              : "NAT type question"}
          </div>
        </QuestionHeader>
        <Question>
          <div className="ques" >
              {ques}
            </div>
          <div className="options">
            {currentQuestion.options && currentQuestion.questionType !== 2 ? (
              currentQuestion.options.map((data, index) => (
                <div className="option" key={index}>
                  <input
                    type={
                      currentQuestion.questionType === 0 ? "radio" : "checkbox"
                    }
                    id={`ques_${currentQuestion.index}_option_${index}`}
                    name={`ques_${currentQuestion.index}_answer`}
                    value={`option_${index}`}
                    checked={answer.includes(index)}
                    onChange={() => handelAnswer(index)}
                  />
                  <label
                    htmlFor={`ques_${currentQuestion.index}_option_${index}`}
                  >
                    <Markdown source={data}></Markdown>
                  </label>
                </div>
              ))
            ) : (
              <>
                <input
                  type="text"
                  value={natAnswer}
                  onChange={(e) => {
                    handelAnswer(e);
                  }}
                  onKeyDown={(e) => e.preventDefault()}
                />
                <div id="container">
                  <ul id="keyboard">
                    <li
                      className="letter"
                      onClick={() => {
                        addCode("1");
                      }}
                    >
                      1
                    </li>
                    <li
                      className="letter"
                      onClick={() => {
                        addCode("2");
                      }}
                    >
                      2
                    </li>
                    <li
                      className="letter"
                      onClick={() => {
                        addCode("3");
                      }}
                    >
                      3
                    </li>
                    <li
                      className="letter clearl"
                      onClick={() => {
                        addCode("4");
                      }}
                    >
                      4
                    </li>
                    <li
                      className="letter"
                      onClick={() => {
                        addCode("5");
                      }}
                    >
                      5
                    </li>
                    <li
                      className="letter"
                      onClick={() => {
                        addCode("6");
                      }}
                    >
                      6
                    </li>
                    <li
                      className="letter clearl"
                      onClick={() => {
                        addCode("7");
                      }}
                    >
                      7
                    </li>
                    <li
                      className="letter "
                      onClick={() => {
                        addCode("8");
                      }}
                    >
                      8
                    </li>
                    <li
                      className="letter"
                      onClick={() => {
                        addCode("9");
                      }}
                    >
                      9
                    </li>
                    <li
                      className="letter clearl"
                      onClick={() => {
                        addCode("0");
                      }}
                    >
                      0
                    </li>
                    <li
                      className="letter"
                      onClick={() => {
                        addCode(".");
                      }}
                    >
                      .
                    </li>
                    <li className="letter "onClick={() => {addCode("-");}}>-</li>
                    <li className="delete clearl"onClick={() => {addCode("clear");}}>Clear</li>
                    <li className="letter " onClick={() => {addCode("del");}}>&lt;-</li>
                  </ul>
                </div>
              </>
            )}
          </div>
        </Question>
      </QuestionContainer>
    )
  );
};
const SectionHeader = styled.div`
  width: 100%;
  padding: 12px 16px;
  display: flex;
  align-items: center;
  justify-content: space-between;
  @media (max-width: 700px) {
    padding: 12px 8px;
  }
  @media (max-width: 650px) {
    padding: 12px 4px;
  }
  @media (max-width: 400px) {
    padding: 10px 0px;
  }
`;

const QuestionContainer = styled.div`
  width: 100%;
  margin-top: 12px;
`;
const QuestionHeader = styled(SectionHeader)`
  border-bottom: 1px solid #1bbc9b;
  color: #1bbc9b;
  .number {
    font-size: 20px;
    @media (max-width: 700px) {
      font-size: 16px;
    }
  }
  .type {
    font-size: 16px;
    @media (max-width: 700px) {
      font-size: 14px;
    }
  }
`;
const Question = styled.div`
  width: 100%;
  color: #1bbc9b;
  padding: 12px 16px;
  @media (max-width: 700px) {
    padding: 12px 12px;
  }
  @media (max-width: 650px) {
    padding: 12px 4px;
  }
  @media (max-width: 400px) {
    padding: 12px 0px;
  }
  .ques {
    color: black;
    font-size: 20px;
    font-weight: 500;
    margin: 12px 0px;
    @media (max-width: 700px) {
      font-size: 16px;
    }
  }
  .options {
    color: black;
    .option {
      margin: 16px 0px;
      input {
        margin-right: 24px;
        cursor: pointer;
      }
      label {
        font-size: 20px;
        font-weight: 500;
        cursor: pointer;
        @media (max-width: 700px) {
          font-size: 16px;
        }
      }
    }
  }

  #container {
    margin-bottom: 11rem;
    width: 760px;
  }
  #keyboard {
    margin: 0;
    padding: 0;
    list-style: none;
  }
  #keyboard li {
    float: left;
    margin: 0 5px 5px 0;
    width: 60px;
    height: 60px;
    font-size: 24px;
    line-height: 60px;
    text-align: center;
    background: #fff;
    border: 1px solid #000000;
    border-radius: 5px;
    @media (max-width: 700px) {
      width: 50px;
      height: 50px;
    }
  }
  .capslock,
  .tab,
  .left-shift,
  .clearl,
  .switch {
    clear: left;
  }
  #keyboard .tab,
  #keyboard .delete {
    width: 125px;
  }
  #keyboard .capslock {
    width: 80px;
  }
  #keyboard .return {
    width: 90px;
  }
  #keyboard .left-shift {
    width: 70px;
  }

  #keyboard .switch {
    width: 90px;
  }
  #keyboard .rightright-shift {
    width: 109px;
  }
  .lastitem {
    margin-right: 0;
  }
  .uppercase {
    text-transform: uppercase;
  }
  #keyboard .space {
    float: left;
    width: 556px;
  }
  #keyboard .switch,
  #keyboard .space,
  #keyboard .return {
    font-size: 16px;
  }
  .on {
    display: none;
  }
  #keyboard li:hover {
    position: relative;
    top: 1px;
    left: 1px;
    border-color: #000000;
    cursor: pointer;
  }
`;
const MainCont = styled.div`
  position: relative;
  width: 100%;
  height: 100%;
  padding: 0px 12px;
  display: flex;
  flex-direction: column;
  align-items: flex-start;
  grid-column: ${(props) => (props.isSideBarOpen.byClick ? "1/2" : "1/3")};
  grid-column: ${(props) => (props.isSideBarOpen.bySize ? "" : "1/3")};
`;
const Toggle = styled.div`
  position: absolute;
  top: 70px;
  right: 0px;
  width: 30px;
  height: 36px;
  border-top-left-radius: 50%;
  border-bottom-left-radius: 50%;
  background-color: white;
  cursor: pointer;
  display: flex;
  justify-content: center;
  align-items: center;
  color: #1bbc9b;
  border: 2px solid #1bbc9b;
  border-right: none;
  /* transform: translateX(100%); */
  svg {
    margin-left: 12px;
  }
`;
const Button = styled(OutlinedButton)`
  margin-bottom: 32px;
  margin-right: 12px;
  padding: 10px 16px;
  transition: all 0.3s ease-in-out;
  @media (max-width: 850px) {
    padding: 8px 8px;
    margin-right: 10px;
  }
  @media (max-width: 780px) {
    padding: 8px 4px;
    margin-right: 8px;
  }
  @media (max-width: 750px) {
    margin-right: 6px;
    font-size: 14px;
    margin-bottom: 24px;
  }
  @media (max-width: 430px) {
    font-size: 12px;
  }
  @media (max-width: 360px) {
    font-size: 10px;
  }

  &:hover {
    background: linear-gradient(
      86.94deg,
      #1bbc9b 0%,
      #1bbc9b 0.01%,
      #16a086 100%
    );
    color: white;
  }
`;
const ButtonContainer = styled.div`
  margin-top: auto;
  display: flex;
  padding: 0px 12px;
  @media (max-width: 680px) {
    width: 100%;
    display: grid;
    grid-template-columns: 1fr 1fr;
    gap: 8px;
    align-items: center;
    div {
      width: 100%;
      justify-self: center;
      margin-right: 0px;
      margin-bottom: 0px;
      justify-content: center;
      &:nth-child(2n + 1) {
        grid-column: 1/2;
      }
      &:nth-child(2n) {
        grid-column: 2/3;
      }
    }
  }
  @media (max-width: 380px) {
    padding: 0px 0px;
  }
`;

const SectionButton = styled.button`
  padding: 6px 16px;
  font-size: 16px;
  background-color: #e5f8ff;
  border: 2px solid ${(props) => (props.active ? "#1bbc9b" : "#e5f8ff")};
  border-radius: 4px;
  color: #1bbc9b;
  font-weight: 500;
  cursor: pointer;
  margin: 0px 16px;
  .laptop {
    display: inline-block;
  }
  .phone {
    display: none;
  }
  @media (max-width: 750px) {
    margin: 0px 8px;
  }
  @media (max-width: 700px) {
    margin: 0px 4px;
    padding: 6px 12px;
  }
  @media (max-width: 650px) {
    margin: 0px 4px;
    padding: 6px 24px;
    .laptop {
      display: none;
    }
    .phone {
      display: inline-block;
    }
  }
  @media (max-width: 600px) {
    font-size: 14px;
  }
  @media (max-width: 500px) {
    padding: 6px 16px;
  }
  @media (max-width: 450px) {
    padding: 6px 10px;
  }
  @media (max-width: 400px) {
    padding: 2px 8px;
    font-size: 12px;
    margin: 0px 2px;
  }
`;
const StyledLabel = styled.label`
  color: #1bbc9b;
  font-size: 16px;
  @media (max-width: 600px) {
    font-size: 14px;
  }
  @media (max-width: 400px) {
    font-size: 12px;
  }
`;
const StyledSelect = styled.select`
  padding: 8px 16px;
  background-color: #e5f8ff;
  border: 2px solid #e5f8ff;
  text-transform: uppercase;
  outline: none;
  border: none;
  margin-left: 8px;
  color: #1bbc9b;
  font-size: 16px;
  @media (max-width: 600px) {
    font-size: 14px;
  }
  @media (max-width: 500px) {
    padding: 8px 12px;
  }
  @media (max-width: 450px) {
    padding: 8px 8px;
  }
  @media (max-width: 400px) {
    padding: 4px 6px;
    font-size: 12px;
  }
  @media (max-width: 350px) {
    font-size: 10px;
  }

  option {
    text-transform: uppercase;
    outline: none;
    border: none;
  }
`;
const MarkingScheme = styled.div`
  display: flex;
  align-items: center;
  padding: 12px 16px;
  @media (max-width: 700px) {
    padding: 12px 8px;
  }
  @media (max-width: 650px) {
    padding: 12px 4px;
  }
  @media (max-width: 400px) {
    padding: 12px 0px;
  }

  div {
    margin: 0px 8px;
    color: #1bbc9b;
    font-size: 16px;
    font-weight: 500;
    @media (max-width: 700px) {
      margin: 0px 4px;
    }
    @media (max-width: 650px) {
      font-size: 14px;
      .hide-for-small-screen {
        display: none;
      }
    }
    @media (max-width: 400px) {
      font-size: 12px;
    }
    span {
      margin-left: 8px;
      @media (max-width: 700px) {
        margin-left: 4px;
      }
    }
  }
`;
export default Main;
